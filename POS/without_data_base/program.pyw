...
def modify_item_price(selected_item, modified_price, window):
    ...
    new_item.update_item(order_data)
    preload()
    window.destroy()
...
def add_ordered_items():
    ...
    for x in selected_item:
        the_item = list(items_list_sorted.keys())[x]
        quantity = int(quantity_spinbox.get())
        if the_item in order_text_box_name.get(0, "end"):
            if messagebox.askyesno(f"{the_item} Already in Order",
                                   "This item is already in the order.\nAdd more quantity?"):
                item_index = order_text_box_name.get(0, "end").index(the_item)
                ...
                order_text_box_name.insert(END, the_item)
...
def preload():
    ...
    if items_list:
        ...
        for x in items_list_sorted.keys():
            ...
            # items_price_list_box.insert(END, items_list_sorted[x])
