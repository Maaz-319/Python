...
# Check if studentrecordsystemdata file is not absent or corrupted
try:
    from studentrecordsystemdata import names
    from studentrecordsystemdata import phones
    from studentrecordsystemdata import f_names
    from studentrecordsystemdata import roll_ns
except ImportError:
    file = open('studentrecordsystemdata.py', 'w')
    file.write('names=[]')
    file.write('\nphones=[]')
    file.write('\nf_names=[]')
    file.write('\nroll_ns=[]')
        ...
        if student_roll == 0:
            ...
            if student_roll in roll_ns:
                ...
                if len(str(student_phone)) != 11:
                    ...
                    studentrecordsystemdata_file = open('studentrecordsystemdata.py', 'w')
                    names.append(student_name)
                    f_names.append(student_f_name)
                    phones.append(student_phone)
                    roll_ns.append(student_roll)
                    studentrecordsystemdata_file.write('names=' + str(names))
                    studentrecordsystemdata_file.write('\nf_names=' + str(f_names))
                    studentrecordsystemdata_file.write('\nphones=' + str(phones))
                    studentrecordsystemdata_file.write('\nroll_ns=' + str(roll_ns))
                    studentrecordsystemdata_file.close()
...
# New studentrecordsystemdata Button
Button(main_page, text="Add Student", font=("Microsoft YaHei UI Light", 10, 'bold'), borderwidth=1,
       background="Light Green", command=create_new_student).place(x=80, y=150)
