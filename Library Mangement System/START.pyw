...
from class_librarian import Librarian
import os
...
error_color = "#ff5252"
admin_code = 7391
librarian_login = db_librarian.get_all_items_name() if db_librarian.get_all_items_name() else []
new_librarian = None
...
# -------------------------------------- Function to log in ---------------------------------------------
def login(_=None):
    global current_librarian, theme
    ...
    if username == "" or password == "":  # for empty entries
        ...
        return
    elif username not in librarian_login:  # for unregistered cashier
        messagebox.showerror("Error", "Username not found")
        username_entry.delete(0, tk.END)
        password_entry.delete(0, tk.END)
        return
    elif db_librarian.get_item_by_name(username)[2] != password:  # for wrong password
    ...
    current_librarian = username
    # write current cashier to file
    se.update_settings(current_librarian, theme)
...
def signup(username, password, admin_pass, window_2):
    global new_librarian, current_librarian
    ...
    if username == "" or password == "":  # for empty fields
        messagebox.showerror("Error", "Please enter both username and password")
        return
    elif username in librarian_login:  # for already registered cashier
    ...
    new_librarian = Librarian(username, password)  # create cashier object
    new_librarian.add_librarian()  # save cashier to database

    current_librarian = None
    se.update_settings(current_librarian, "united")
...
window.title("Welcome to LMS Login")
